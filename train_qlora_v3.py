...
v3: Uses multi-GPU model parallelism (no quantization) instead of broken 4-bit QLoRA.
Since the RX 6600 XT cards don't support 4-bit bitsandbytes properly on ROCm,
we use fp16 across multiple GPUs with gradient checkpointing.
LoRA is applied on top of the fp16 model (still efficient, just not 4-bit quantized).
"""
...
def main():
    print("=" * 60)
    print("LoRA Fine-Tuning: qwen2.5:3b → trader-v2 (fp16 + multi-GPU)")
    print("=" * 60)
    ...
    # LoRA parameters (slightly larger rank since we're in fp16)
    LORA_RANK = 16
    LORA_ALPHA = 32
    LORA_DROPOUT = 0.05
    TARGET_MODULES = [
    ...
    # Training parameters — conservative for 4× 8GB GPUs
    LEARNING_RATE = 2e-4
    NUM_EPOCHS = 3
    ...
    # Calculate device map for multi-GPU
    # Split model across 2 GPUs (GPU 0 and GPU 1, leaving GPU 2/3 free)
    # qwen2.5:3b has ~36 layers, so 18 layers per GPU
    num_gpus = min(torch.cuda.device_count(), 2)  # Use 2 GPUs for model
    
    print(f"[INFO] Distributing model across {num_gpus} GPUs...")
    
    # Use device_map="auto" without quantization (should work since fp16 doesn't need bitsandbytes)
