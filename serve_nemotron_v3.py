...
Serves merged model on port 11435 (separate from Ollama on 11434).
API-compatible with Ollama for easy integration.

Usage:
  python3 serve_nemotron_v3.py
  
Environment:
  MODEL_PATH - path to merged model (default: ./output/nemotron-trader-v3/merged)
  PORT - API port (default: 11435)
  CUDA_VISIBLE_DEVICES - which GPU to use (default: 0)
  HSA_OVERRIDE_GFX_VERSION - ROCm gfx version (default: 10.3.0 for gfx1032)
"""
...
MODEL_PATH = os.environ.get('MODEL_PATH', './output/nemotron-trader-v3/merged')
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/api/tags':
            ...
            self.end_headers()
            self.wfile.write(json.dumps({
                'models': [{'name': 'nemotron-trader-v3'}]
            }).encode())
        else:
            self.send_response(200)
            self.send_header('Content-Type', 'application/json')
