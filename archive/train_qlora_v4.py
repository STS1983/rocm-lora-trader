#!/usr/bin/env python3
"""
QLoRA Fine-Tuning Script for qwen2.5:3b Trading Model — v4 (Stable Single-GPU)
Runs on MortySmith with 1× RX 6600 XT (ROCm 7.0)

v4: Single-GPU only — no multi-GPU sharding that caused hipErrorLaunchFailure.
Uses GPU 0 only, with gradient checkpointing + fp16.
Saves checkpoints every 50 steps so progress is preserved on crash.
"""
...
# Force single GPU, set HSA vars for stability
os.environ["HSA_OVERRIDE_GFX_VERSION"] = "10.3.0"
os.environ["HIP_VISIBLE_DEVICES"] = "0"       # Only GPU 0
os.environ["CUDA_VISIBLE_DEVICES"] = "0"       # Only GPU 0
os.environ["HSA_ENABLE_SDMA"] = "0"            # Disable SDMA (stability fix)
os.environ["PYTORCH_HIP_ALLOC_CONF"] = "garbage_collection_threshold:0.6,max_split_size_mb:128"

def main():
    print("=" * 60)
    print("LoRA Fine-Tuning: qwen2.5:3b → trader-v2 (v4 Single-GPU)")
    print("=" * 60)
    ...
    # Training parameters — single GPU, conservative
    LEARNING_RATE = 2e-4
    NUM_EPOCHS = 3
    BATCH_SIZE = 1                  # per GPU (only 1 GPU)
    ...
    print(f"[CONFIG] Strategy: Single-GPU (GPU 0 only)")
    ...
    print(f"[CONFIG] Effective batch: {BATCH_SIZE * GRADIENT_ACCUMULATION_STEPS}")
    print(f"[CONFIG] Max sequence length: {MAX_SEQ_LENGTH}")
    print(f"[CONFIG] Save every {SAVE_STEPS} steps")
    ...
    # ============ Load Model ============
    print("\n[STEP 2] Loading base model (fp16, single GPU)...")
    ...
    # Single GPU — no device_map needed, just put model on cuda:0
    ...
    print(f"[INFO] Model loaded on cuda:0")
    print(f"[INFO] GPU memory: {torch.cuda.memory_allocated(0) / 1e9:.2f} GB used")
    ...
    print(f"[INFO] GPU memory after LoRA: {torch.cuda.memory_allocated(0) / 1e9:.2f} GB used")
    ...
    training_args = TrainingArguments(
        ...
        # KEY FIX: Save checkpoints every 50 steps (not just epoch end)
        eval_strategy="steps",
        ...
        dataloader_num_workers=0,     # Single GPU, no multi-process
    ...
    # ============ Train ============
    print("\n[STEP 5] Starting training (single-GPU, checkpoint every 50 steps)...")
    print("[INFO] Even if it crashes, checkpoints at step 50, 100, 150... are preserved!")
    ...
    try:
        ...
        if checkpoints:
            ...
            print(f"  python3 train_qlora_v4.py --resume_from_checkpoint {latest_checkpoint}")
        else:
            print(f"\n[RECOVERY] No checkpoints found. Training must restart from scratch.")
            print(f"[RECOVERY] Consider reducing MAX_SEQ_LENGTH or increasing SAVE_STEPS frequency.")
